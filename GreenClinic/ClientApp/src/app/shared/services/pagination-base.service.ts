...
import { Subject, ReplaySubject, Observable, empty, BehaviorSubject } from 'rxjs';
import { distinctUntilChanged, merge, switchMap, catchError, filter, finalize } from 'rxjs/operators';
    ...
    this.requestUrl.pipe(
      ...
      switchMap((url: string) => {
        ...
        return this.http.get<PaginationResult<T>>(url)
          .pipe(catchError(er => empty()), finalize(() => this.loadingSubject.next(false)));
      }),
      filter(r => r != null))
      .subscribe(result => {
